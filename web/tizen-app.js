...
(function () {
  ...
  // como página (iframe/embed), não como recurso de mídia direto.
  function isBloggerUrl(url) {
    return !!url && (url.indexOf('blogger.com/video.g') !== -1 || url.indexOf('googlevideo.com') !== -1);
  }

  // Toca um vídeo do Blogger dentro de um iframe (evita o 403 causado
  // por tentar carregar o video.g diretamente como recurso de mídia).
  function playBloggerVideo(url) {
    var container = document.getElementById('player-video-container');
    ...
    // O Blogger não expõe timeupdate/duration via JS — esconder os
    // controles que dependem disso (barra de progresso, tempo, play/pause)
  ...
  function playVideo(url, title, itemId, cat, epIdx) {
    ...
    // Destruir player anterior (HLS e/ou iframe do Blogger)
    destroyHls();
    ...
    if (isBloggerUrl(url)) {
      // ── Blogger: precisa de iframe, não de <video src=""> ──
      playBloggerVideo(url);
      showControls();
      // Sem timeupdate/duration não há progresso para restaurar/salvar
    } else {
      // ── MP4 / HLS normais ──
    ...
    // Próximo episódio (funciona tanto para vídeo normal quanto Blogger)
    var item    = null;
    var items   = vodData[cat] || [];
  ...
  function bindPlayer() {
    ...
    btnBack.onclick = function () {
      if (isBloggerMode && bloggerIframe && bloggerIframe.contentWindow) {
        bloggerIframe.contentWindow.postMessage({ event: 'command', func: 'seek', args: -10 }, '*');
        showOsd('⏪ -10s');
        return;
      }
      video.currentTime = Math.max(0, video.currentTime - 10); showOsd('⏪ -10s');
    };
    btnFwd.onclick  = function () {
      ...
      video.currentTime = Math.min(video.duration || 0, video.currentTime + 10); showOsd('⏩ +10s');
    };
    ...
    wrap.addEventListener('touchend', function (e) {
      ...
      if (Math.abs(dx) > 60) {
        if (isBloggerMode && bloggerIframe && bloggerIframe.contentWindow) {
          bloggerIframe.contentWindow.postMessage({ event: 'command', func: 'seek', args: dx > 0 ? -10 : 10 }, '*');
          showOsd(dx > 0 ? '⏪ -10s' : '⏩ +10s');
        } else if (dx > 0) { video.currentTime = Math.max(0, video.currentTime - 10); showOsd('⏪ -10s'); }
    ...
    document.addEventListener('keydown', function (e) {
      ...
      switch (key) {
        ...
        case 415: // Tizen Play
          e.preventDefault();
          if (!isBloggerMode) { video.paused ? video.play() : video.pause(); }
          showControls();
          break;
        case 19:  // Tizen Pause
          e.preventDefault();
          if (!isBloggerMode) { video.pause(); }
          showControls();
          break;
        case 37:  // Left
