...
def generate_html_with_correct_paths(base_dir, data):
    ...
    # =========================================================
    # ATENÇÃO: Todo o JavaScript abaixo usa {{ }} para escapar
    # chaves dentro do f-string Python. Variáveis Python usam
    # {var} normalmente; chaves literais JS usam {{ e }}.
    # =========================================================
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .see-all-link {{
            color: #e50914; text-decoration: none; font-size: 0.9rem;
            padding: 5px 10px; border-radius: 3px; transition: background 0.3s;
        }}
        ...
        .owl-carousel .item-card {{ margin: 0 5px; width: auto; }}
        .item-card {{
        ...
        .modal-body {{ padding: 30px; position: relative; z-index: 1; }}
        ...
        .continue-watching .item-card {{ width: 160px !important; }}
        .continue-watching .item-info {{ padding: 10px; }}
        .continue-watching .item-title {{ font-size: 0.9rem; }}
        .continue-watching .item-meta {{ font-size: 0.7rem; }}
        .continue-watching .progress-bar {{
            position: absolute; bottom: 0; left: 0; width: 100%;
            height: 4px; background: rgba(255,255,255,0.3); z-index: 3;
        }}
        .continue-watching .progress-fill {{ height: 100%; background: #e50914; width: 0%; }}
        ...
        @media (max-width: 768px) {{
            .continue-watching .item-poster {{ height: 120px !important; }}
            .continue-watching .item-card {{ width: 120px !important; }}
        }}
    </style>
</head>
<body>
    ...
    <script>
    // Dicionário de logos dos canais (injetado pelo Python)
    ...
    function initFallbackScroll() {{
        ...
        $('.owl-carousel').css({{ 'display': 'flex', 'overflow-x': 'auto', 'gap': '10px', 'padding': '10px 0' }});
    ...
    function initCarousels() {{
        setTimeout(function() {{
            $('.owl-carousel').each(function() {{
                ...
                const isContinue = carouselId === 'carousel-continue';
                const itemCount = isContinue ? 8 : 7;
                const marginVal = isContinue ? 8 : 10;
                ...
                if (!$carousel.data('owlCarousel')) {{
                    $carousel.owlCarousel({{
                        ...
                        dots: false,
                        responsive: {{
                            0:    {{ items: isContinue ? 3 : 2 }},
                            480:  {{ items: isContinue ? 4 : 3 }},
                            640:  {{ items: isContinue ? 5 : 4 }},
                            768:  {{ items: isContinue ? 6 : 5 }},
                            1024: {{ items: isContinue ? 7 : 6 }},
                            1280: {{ items: isContinue ? 8 : 7 }}
                        }},
                ...
                $('.next-' + carouselId).off('click').on('click', function(e) {{
                    e.preventDefault();
                    $carousel.trigger('next.owl.carousel');
                }});
                $('.prev-' + carouselId).off('click').on('click', function(e) {{
                    e.preventDefault();
    ...
    // =====================================================
    // CORREÇÃO PRINCIPAL: displayContent com {{ }} corretos
    // =====================================================
    function displayContent() {{
        ...
        if (continueWatching.length > 0) {{
            ...
            html += '<a class="nav-btn prev-continue"><i class="fas fa-chevron-left"></i></a>';
            html += '<a class="nav-btn next-continue"><i class="fas fa-chevron-right"></i></a>';
            html += '</div></div>';
            ...
            continueWatching.forEach(function(item) {{
                ...
                html += '<div class="item-card" onclick="openModal(\'' + item.category + '\', \'' + item.id + '\')">';
                html += '<img src="' + item.poster + '" alt="' + item.title + '" class="item-poster"';
                html += ' onerror="this.onerror=null; this.src=\'' + RAW_BASE + '/assets/Capas/default.jpg\';">';
        ...
        console.log('📺 Canais de TV:', vodData.tv ? vodData.tv.length : 0);
        ...
        categoryOrder.forEach(function(category) {{
            ...
            items.forEach(function(item) {{
                ...
                if (category === 'tv') {{
                    ...
                    poster = (window.channelsDict && window.channelsDict[nomeLimpo])
                        ? window.channelsDict[nomeLimpo]
                ...
                html += '<div class="item-card" onclick="openModal(\'' + category + '\', \'' + item.id + '\')">';
                html += '<img src="' + poster + '" alt="' + item.title + '" class="item-poster"';
                html += ' onerror="this.onerror=null; this.src=\'' + RAW_BASE + '/assets/Capas/default.jpg\';">';
                html += '<div class="item-info">';
                html += '<div class="item-title">' + item.title + '</div>';
    ...
    function openModal(category, itemId) {{
        ...
        if (category === 'tv') {{
            const nomeLimpo = item.title.toLowerCase().replace(/[^a-z0-9]/g, '');
            posterUrl = (window.channelsDict && window.channelsDict[nomeLimpo])
                ? window.channelsDict[nomeLimpo]
                : RAW_BASE + '/assets/Capas/tv_default.jpg';
        }} else {{
            const nomeArquivo = item.poster ? item.poster.split('/').pop() : 'default.jpg';
        ...
        let modalHeaderHtml = '<div class="modal-backdrop" style="background-image: url(\'' + posterUrl + '\')"></div>';
        if (category === 'tv') {{
            modalHeaderHtml += '<div style="position:absolute;top:30px;left:30px;background:#e50914;color:white;padding:8px 16px;border-radius:4px;font-weight:bold;">🔴 AO VIVO</div>';
        }}
        modalHeaderHtml += '<button class="play-button" onclick="playFirstEpisode(\'' + category + '\', \'' + item.id + '\')" style="position:absolute;bottom:30px;left:30px;">';
        modalHeaderHtml += '<i class="fas fa-play"></i> ' + (category === 'tv' ? 'Assistir ao Vivo' : 'Assistir') + '</button>';

        let modalBodyHtml = '<h2 class="modal-title">' + item.title + '</h2>';
        modalBodyHtml += '<div class="modal-meta">';
        modalBodyHtml += '<span>' + (category === 'filmes' ? 'Filme' : category === 'tv' ? 'Canal de TV' : 'Série') + '</span>';
        if (category === 'tv') modalBodyHtml += '<span>🔴 Ao Vivo</span>';
        if (item.episodes) modalBodyHtml += '<span>' + item.episodes.length + (category === 'tv' ? ' canais' : ' episódios') + '</span>';
        modalBodyHtml += '</div>';
        modalBodyHtml += '<button class="play-button" onclick="playFirstEpisode(\'' + category + '\', \'' + item.id + '\')">';
        modalBodyHtml += '<i class="fas fa-play"></i> ' + (category === 'tv' ? 'Assistir ao Vivo' : 'Assistir') + '</button>';

        if (category === 'tv' && item.episodes && item.episodes.length > 0) {{
            modalBodyHtml += '<div class="episodes-section"><h3 style="margin-bottom:20px;font-size:1.3rem;">Canais Disponíveis</h3>';
            modalBodyHtml += '<div class="episode-list">';
            item.episodes.forEach(function(ep, index) {{
                modalBodyHtml += '<div class="episode-item" onclick="playEpisode(\'' + ep.url + '\', \'' + item.title + ' - ' + (ep.title || '') + '\', \'' + item.id + '\', \'' + category + '\', ' + index + ')">';
                modalBodyHtml += '<div class="episode-number">' + (index + 1) + '</div>';
                modalBodyHtml += '<div class="episode-info"><div class="episode-title">' + (ep.title || item.title) + '</div>';
                modalBodyHtml += '<div class="episode-duration">🔴 Ao Vivo</div></div></div>';
            }});
    ...
    function playFirstEpisode(category, itemId) {{
        ...
        if (item.episodes && item.episodes.length > 0) {{
            playEpisode(item.episodes[0].url, item.title + ' - ' + item.episodes[0].title, item.id, category, 0);
        }} else if (item.seasons && item.seasons.length > 0 && item.seasons[0].episodes.length > 0) {{
            const ep = item.seasons[0].episodes[0];
            playEpisode(ep.url, item.title + ' - Temp 1 - ' + ep.title, item.id, category, 0);
        }}
    }}
